    ...
    <div className="flex flex-col gap-8 p-6 bg-white shadow-md rounded-lg">
      {productOptions.map((option) => (
        <div className="flex flex-col gap-2" key={option.name}>
          <h4 className="text-lg font-semibold text-gray-800">Choose a {option.name}</h4>
          <ul className="flex items-center gap-4">
            {option.choices?.map((choice) => {
              ...
              return option.name === "Color" ? (
                <li
                  className={`w-8 h-8 rounded-full ring-2 ${disabled ? "ring-gray-300" : "ring-gray-500"} relative`}
                  style={{
                    backgroundColor: choice.value,
                    cursor: disabled ? "not-allowed" : "pointer",
                  }}
                ...
                >
                  {selected && (
                    <div className="absolute w-10 h-10 rounded-full border-2 border-gray-700 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                  )}
                  {disabled && (
                    <div className="absolute w-10 h-[2px] bg-red-400 rotate-45 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                  )}
                ...
                <li
                  className={`rounded-md py-1 px-4 text-sm font-medium transition-colors ${
                    disabled
